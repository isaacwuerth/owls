...
function multiFunc (...functions: Function[]) {
  return async function (...params: any) {
    functions.forEach((f: Function) => f(...params))
  }
}
export function DynamicCountry (props: DynamicBaseInputProps): JSX.Element {
  const { label, error, placeholder, errorMessage, helperText, onBlur, onChange, value, fullWidth, disabled } = props
  ...
  async function handleInputChange (event: ChangeEvent<{}>, value: string) {
    ...
    axios('https://restcountries.com/v3.1/translation/' + value).then((response) => {
      const countries: CountryType[] = response.data.map((country: CountryResponse) => {
        return {
          code: country.cca2,
  ...
  useEffect(() => {
    ...
    axios('https://restcountries.com/v3.1/alpha?codes=' + cca2).then((response) => {
      const countries: CountryType[] = response.data.map((country: CountryResponse) => {
        return {
          code: country.cca2,
          label: country.translations.deu?.common ?? country.name,
